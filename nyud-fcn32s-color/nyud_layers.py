...
@file: nyud_layer.py
@time: 2019/1/2 15:59
@desc: python定义的网络层文件，层名称为NYUDSegDataLayer，在trainval.prototxt和test.prototxt文件的第一层指定要使用该层
'''
import caffe
...
class NYUDSegDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # add this-->yelele
        self.class_map = scipy.io.loadmat('{}/data/benchmarkData/metadata/classMapping40.mat'.format(self.nyud_dir))[
            'mapClass'].astype(np.uint8)
    ...
    def load_label(self, idx):
        ...
        label = scipy.io.loadmat('{}/segmentation/img_{}.mat'.format(self.nyud_dir, idx))['groundTruth'][0, 0][0, 0][
            'SegmentationClass'].astype(np.uint16)
